...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    db.get('SELECT id FROM users WHERE username = ?', [username], async (err, row) => {
      ...
      if (row) {
        // Eliminar usuario existente (solo para pruebas)
        db.run('DELETE FROM users WHERE username = ?', [username], async (delErr) => {
          if (delErr) {
            return res.status(500).json({ error: 'Error eliminando usuario existente' });
      }

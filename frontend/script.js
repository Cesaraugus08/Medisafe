...
function renderMeds() {
  ...
  meds.forEach((med, idx) => {
    ...
    // Verificar si fue tomado hoy
    let tomadoHoy = false;
    if (med.lastTaken) {
      const lastTakenDate = new Date(med.lastTaken);
      const lastTakenDay = lastTakenDate.toISOString().slice(0, 10);
      if (lastTakenDay === today) tomadoHoy = true;
    }
    li = document.createElement('li');
    li.className = classes.join(' ');
    li.innerHTML = `<strong>${med.name}</strong> <span>Dosis: ${med.dose}</span> <span>Frecuencia: ${med.freq}</span> <span>Horario: ${formatTimeToAMPM(med.time)}</span> <span>Caducidad: ${med.expiry || '-'}</span> <span>Notas: ${med.notes || '-'}</span> <button data-idx="${idx}" class="edit-btn">Editar</button> <button data-idx="${idx}" class="delete-btn">Eliminar</button> <button data-idx="${idx}" class="take-btn btn btn-sm btn-outline-success ms-2" ${tomadoHoy ? 'disabled' : ''}>${tomadoHoy ? 'Tomado hoy' : 'Marcar como tomado'}</button>`;
    li.onclick = function(e) {
      if (e.target.classList.contains('delete-btn') || e.target.classList.contains('edit-btn') || e.target.classList.contains('take-btn')) return;
      showDetailModal(med);
    };
  ...
  document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.onclick = function(e) {
      ...
      const meds = getMeds();
      const med = meds[idx];
      if (confirm(`¿Estás seguro de que deseas eliminar el medicamento "${med.name}"? Esta acción no se puede deshacer.`)) {
        meds.splice(idx, 1);
        saveMeds(meds);
        renderMeds();
      }
  ...
  document.querySelectorAll('.take-btn').forEach(btn => {
    btn.onclick = function(e) {
      ...
      saveMeds(meds);
      // Marcar recordatorio correspondiente como tomado (si existe para hoy)
      const reminders = getReminders();
      const today = now.getDay();
      let updated = false;
      reminders.forEach(reminder => {
        if (
          reminder.medId === med.id &&
          reminder.days.includes(today)
        ) {
          // Solo marcar si no está ya marcado hoy
          if (!reminder.lastTaken || (new Date(reminder.lastTaken)).toDateString() !== now.toDateString()) {
            reminder.lastTaken = now.toISOString();
            updated = true;
          }
        }
      });
      ...
      renderMeds();
      showNotification(`✅ Medicamento "${med.name}" marcado como tomado hoy`, 'success');
    };
  });
}
...
window.onload = function() {
  // Siempre mostrar la pantalla de login al cargar la página
  showAuth();
...
function startReminderChecker() {
  ...
  // Verificar cada segundo para máxima precisión
  reminderChecker = setInterval(checkReminders, 1000); // 1000ms = 1 segundo
...
function checkReminders() {
  ...
  reminders.forEach(reminder => {
    ...
    // Hora y minuto del recordatorio en minutos desde medianoche
    const reminderTimeMinutes = reminderHour * 60 + reminderMinute;
    const nowMinutes = currentHour * 60 + currentMinute;

    // Si el recordatorio ya fue tomado hoy, no notificar más
    if (reminder.lastTaken) {
      const lastTakenDate = new Date(reminder.lastTaken);
      if (
        lastTakenDate.getDate() === now.getDate() &&
        lastTakenDate.getMonth() === now.getMonth() &&
        lastTakenDate.getFullYear() === now.getFullYear()
      ) {
        return;
      }
    }

    // Notificación exacta a la hora programada
    if (currentHour === reminderHour && currentMinute === reminderMinute) {
      showReminderNotification(med, reminder, 'due');
    }
    // Si han pasado más de 5 minutos pero menos de 1 hora y no se ha marcado como tomado
    else if (nowMinutes > reminderTimeMinutes && nowMinutes - reminderTimeMinutes === 5) {
      showReminderNotification(med, reminder, 'overdue');
    }
    // Si han pasado más de 1 hora y no se ha marcado como tomado
    else if (nowMinutes > reminderTimeMinutes && nowMinutes - reminderTimeMinutes === 60) {
...
function showReminderNotification(med, reminder, status) {
  ...
  if (settings.sound) {
    try {
      let audio = document.getElementById('reminder-audio');
      if (!audio) {
        audio = document.createElement('audio');
        audio.id = 'reminder-audio';
        document.body.appendChild(audio);
      }
      audio.src = settings.soundUrl || DEFAULT_REMINDER_SETTINGS.soundUrl;
      audio.preload = 'auto';
      audio.currentTime = 0;
      audio.play();
    } catch (e) {
      console.warn('No se pudo reproducir el sonido de recordatorio:', e);
    }
  }
  // Vibrar si está activado y el dispositivo lo soporta
  if (settings.vibration && navigator.vibrate) {
    navigator.vibrate(settings.vibrationPattern || DEFAULT_REMINDER_SETTINGS.vibrationPattern);
  }
...
// --- Configuración de alertas de recordatorio ---
const DEFAULT_REMINDER_SETTINGS = {
  sound: true,
  vibration: true,
  soundUrl: 'https://cdn.pixabay.com/audio/2022/07/26/audio_124bfae1b2.mp3', // Sonido por defecto
  vibrationPattern: [500, 200, 500] // Vibrar: 500ms, pausa 200ms, vibrar 500ms
};

function getReminderSettings() {
  return JSON.parse(localStorage.getItem('reminderSettings') || JSON.stringify(DEFAULT_REMINDER_SETTINGS));
}
...
// Función para mostrar el modal de configuración de alertas
function showReminderSettingsModal() {
          ...
          <div class="modal-body">
            <form id="reminder-settings-form">
              <div class="form-check mb-2">
                <input class="form-check-input" type="checkbox" id="reminder-sound" ${settings.sound ? 'checked' : ''}>
                <label class="form-check-label" for="reminder-sound">Activar sonido</label>
              </div>
              <div class="mb-2">
                <label for="reminder-sound-url" class="form-label">URL del sonido</label>
                <input type="text" class="form-control" id="reminder-sound-url" value="${settings.soundUrl}">
              </div>
              <div class="form-check mb-2">
                <input class="form-check-input" type="checkbox" id="reminder-vibration" ${settings.vibration ? 'checked' : ''}>
                <label class="form-check-label" for="reminder-vibration">Activar vibración</label>
              </div>
              <div class="mb-2">
                <label for="reminder-vibration-pattern" class="form-label">Patrón de vibración (ms, separado por comas)</label>
                <input type="text" class="form-control" id="reminder-vibration-pattern" value="${settings.vibrationPattern.join(',')}">
              </div>
            </form>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
            <button type="button" class="btn btn-primary" id="save-reminder-settings">Guardar</button>
          </div>
        </div>
      </div>
    `;
  ...
  // Guardar cambios
  setTimeout(() => {
    document.getElementById('save-reminder-settings').onclick = function() {
      const sound = document.getElementById('reminder-sound').checked;
      const soundUrl = document.getElementById('reminder-sound-url').value.trim();
      const vibration = document.getElementById('reminder-vibration').checked;
      const vibrationPattern = document.getElementById('reminder-vibration-pattern').value.split(',').map(x => parseInt(x.trim(), 10)).filter(x => !isNaN(x));
      saveReminderSettings({ sound, soundUrl, vibration, vibrationPattern });
...
// Agregar botón de configuración de alertas en la interfaz (puedes moverlo donde prefieras)
document.addEventListener('DOMContentLoaded', function() {
  let btn = document.getElementById('reminder-settings-btn');
  if (!btn) {
    btn = document.createElement('button');
    btn.id = 'reminder-settings-btn';
    btn.className = 'btn btn-outline-secondary btn-sm ms-2';
    btn.innerHTML = '<i class="bi bi-gear"></i> Alertas';
    const controls = document.querySelector('.accessibility-controls');
    if (controls) controls.appendChild(btn);
  }
  btn.onclick = showReminderSettingsModal;
});

// --- Sección de estadísticas de cumplimiento ---
function renderComplianceStats() {
  const stats = getReminderStats();
  let statsSection = document.getElementById('compliance-stats');
  // Insertar dentro de la sección de medicamentos si existe
  let medListSection = document.getElementById('med-list-section');
  if (!statsSection) {
    statsSection = document.createElement('div');
    statsSection.id = 'compliance-stats';
    statsSection.className = 'test-section';
    if (medListSection) {
      medListSection.insertBefore(statsSection, medListSection.firstChild.nextSibling); // Debajo de la barra de accesibilidad
    } else {
      // Fallback: en el contenedor principal
      const container = document.querySelector('.container') || document.body;
      container.insertBefore(statsSection, container.firstChild.nextSibling);
    }
  }
  statsSection.innerHTML = `
    <h3><i class="bi bi-bar-chart-line me-2"></i>Tasa de Cumplimiento de Medicamentos (Hoy)</h3>
    <div class="mb-2">
      <div class="progress" style="height: 30px;">
        <div class="progress-bar bg-success" role="progressbar" style="width: ${stats.compliance}%;" aria-valuenow="${stats.compliance}" aria-valuemin="0" aria-valuemax="100">
          ${stats.compliance}%
        </div>
      </div>
    </div>
    <ul class="list-group mb-2">
      <li class="list-group-item"><strong>Total de tomas programadas:</strong> ${stats.total}</li>
      <li class="list-group-item text-success"><strong>Tomadas:</strong> ${stats.taken}</li>
      <li class="list-group-item text-warning"><strong>Pendientes:</strong> ${stats.total - stats.taken - stats.overdue}</li>
      <li class="list-group-item text-danger"><strong>Atrasadas:</strong> ${stats.overdue}</li>
    </ul>
    <small class="text-muted">La tasa de cumplimiento se calcula con base en los recordatorios marcados como tomados hoy.</small>
  `;
  // Mostrar solo si el usuario está autenticado y en la vista principal
  if (medListSection && medListSection.classList.contains('hidden')) {
...
// Actualizar estadísticas al cargar y cada vez que cambien los recordatorios
function setupComplianceStatsAutoUpdate() {
  renderComplianceStats();
  // Actualizar cada vez que se marque un recordatorio como tomado o se modifiquen recordatorios
  const origSaveReminders = saveReminders;
  window.saveReminders = function() {
    origSaveReminders.apply(this, arguments);
    renderComplianceStats();
  };
  // También actualizar al marcar como tomado
  const origMarkReminderAsTaken = markReminderAsTaken;
  window.markReminderAsTaken = function() {
    origMarkReminderAsTaken.apply(this, arguments);
    renderComplianceStats();
  };
...
document.addEventListener('DOMContentLoaded', setupComplianceStatsAutoUpdate);

// --- Asistente Virtual Flotante SOLO LOCAL, sin IA Gemini ---
// (Eliminadas variables y funciones de Gemini)
// ... existing code ...
  // Lógica de chat SOLO LOCAL
  chatForm.onsubmit = function(e) {
    e.preventDefault();
    const userMsg = chatInput.value.trim();
    if (!userMsg) return;
    addMessage(userMsg, 'user');
    let localResp = getAssistantResponse(userMsg);
    setTimeout(() => {
      if (localResp !== null) {
        addMessage(localResp, 'assistant');
      } else {
        addMessage('No tengo conexión con la IA, pero puedo ayudarte con las funciones principales de MediSafe. Pregúntame sobre agregar medicamentos, recordatorios, metas, etc.', 'assistant');
      }
    }, 400);
    chatInput.value = '';
  };
...
// --- Mostrar/ocultar Asistente Virtual Flotante ---
document.addEventListener('DOMContentLoaded', function() {
  const fab = document.getElementById('assistant-fab');
  const chatContainer = document.getElementById('assistant-chat-container');
  if (fab && chatContainer) {
    fab.onclick = function() {
      if (chatContainer.style.display === 'none' || chatContainer.style.display === '') {
        // Si el chat no está inicializado, lo creamos
        if (!chatContainer.hasChildNodes()) {
          chatContainer.innerHTML = `
            <div style="background:#0d6efd;color:#fff;padding:1rem;display:flex;align-items:center;justify-content:space-between;">
              <span><i class="bi bi-robot me-2"></i>Asistente Virtual</span>
              <button id="assistant-close-btn" style="background:none;border:none;color:#fff;font-size:1.5rem;line-height:1;cursor:pointer;">&times;</button>
            </div>
            <div id="assistant-messages" style="height:260px;overflow-y:auto;padding:1rem;background:#f8f9fa;"></div>
            <form id="assistant-chat-form" style="display:flex;border-top:1px solid #dee2e6;background:#fff;">
              <input id="assistant-chat-input" type="text" placeholder="Escribe tu mensaje..." autocomplete="off" style="flex:1;border:none;padding:0.75rem;font-size:1rem;outline:none;">
              <button type="submit" style="background:#0d6efd;color:#fff;border:none;padding:0 1.25rem;font-size:1.25rem;cursor:pointer;"><i class="bi bi-send"></i></button>
            </form>
          `;
        }
        ...
        // Cerrar
        document.getElementById('assistant-close-btn').onclick = function() {
        ...
        chatForm.onsubmit = function(e) {
          ...
          const userMsg = chatInput.value.trim();
          if (!userMsg) return;
          addAssistantMessage(userMsg, 'user');
          let localResp = getAssistantResponse(userMsg);
          setTimeout(() => {
            if (localResp !== null) {
              addAssistantMessage(localResp, 'assistant');
            } else {
              addAssistantMessage('No tengo conexión con la IA, pero puedo ayudarte con las funciones principales de MediSafe. Pregúntame sobre agregar medicamentos, recordatorios, metas, etc.', 'assistant');
            }
          }, 400);
          chatInput.value = '';
        };
...
// Función para agregar mensajes al chat del asistente
function addAssistantMessage(text, sender) {
  ...
  msg.innerHTML = `<span style="background:${sender==='user'?'#0d6efd':'#e9ecef'};color:${sender==='user'?'#fff':'#212529'};padding:0.5rem 1rem;border-radius:1rem;max-width:80%;display:inline-block;">${text}</span>`;
  msgDiv.appendChild(msg);
  msgDiv.scrollTop = msgDiv.scrollHeight;
}

// --- Lógica de respuestas del Asistente Virtual ---
function getAssistantResponse(userMsg) {
  const msg = userMsg.trim().toLowerCase();
  // Respuestas sobre medicamentos
  if (msg.includes('agregar medicamento') || msg.includes('nuevo medicamento')) {
    return 'Para agregar un medicamento, haz clic en "Agregar Medicamento" y completa el formulario con nombre, dosis, frecuencia y horario.';
  }
  if (msg.includes('eliminar medicamento')) {
    return 'Para eliminar un medicamento, haz clic en el botón "Eliminar" junto al medicamento correspondiente. Se pedirá confirmación.';
  }
  if (msg.includes('editar medicamento') || msg.includes('modificar medicamento')) {
    return 'Para editar un medicamento, haz clic en el botón "Editar" junto al medicamento que deseas modificar.';
  }
  if (msg.includes('ver medicamentos') || msg.includes('lista de medicamentos')) {
    return 'Puedes ver todos tus medicamentos en la sección principal después de iniciar sesión.';
  }
  // Respuestas sobre recordatorios
  if (msg.includes('recordatorio') || msg.includes('notificación')) {
    return 'Puedes gestionar recordatorios desde el botón "Recordatorios". Ahí puedes crear, editar o eliminar recordatorios para tus medicamentos.';
  }
  if (msg.includes('sonido') || msg.includes('vibración')) {
    return 'Puedes personalizar el sonido y la vibración de las notificaciones desde el botón de configuración de alertas (ícono de engranaje).';
  }
  // Respuestas sobre metas
  if (msg.includes('meta') || msg.includes('logro')) {
    return 'Gestiona tus metas y logros desde el botón "Logros". Puedes crear nuevas metas, ver el progreso y marcar metas como completadas.';
  }
  // Ayuda general
  if (msg.includes('ayuda') || msg.includes('cómo') || msg.includes('puedo')) {
    return '¿En qué puedo ayudarte? Puedes preguntarme sobre agregar medicamentos, recordatorios, metas o cómo usar la aplicación.';
  }
  if (msg.includes('cerrar sesión') || msg.includes('logout')) {
    return 'Para cerrar sesión, haz clic en el botón rojo "Cerrar sesión" en la parte superior derecha.';
  }
  if (msg.includes('contacto') || msg.includes('soporte')) {
    return 'Para contactar con soporte técnico, utiliza el botón de soporte en la aplicación.';
  }
  // Saludo
  if (msg.includes('hola') || msg.includes('buenos días') || msg.includes('buenas tardes') || msg.includes('buenas noches')) {
    return '¡Hola! Soy tu asistente virtual de MediSafe. ¿En qué puedo ayudarte hoy?';
  }
  // Despedida
  if (msg.includes('gracias') || msg.includes('adiós') || msg.includes('bye')) {
    return '¡De nada! Si necesitas más ayuda, aquí estaré.';
  }
  // Respuesta por defecto
  return 'No entendí tu mensaje. Puedes preguntarme sobre medicamentos, recordatorios, metas o cómo usar la aplicación.';
}

// Asistente Virtual
(function() {
  const button = document.getElementById('assistant-button');
  const chat = document.getElementById('assistant-chat-container');
  ...
  if (!button || !chat || !closeBtn || !form || !input || !messages) return;
  ...
  function addMessage(text, from) {
    ...
    div.className = from === 'assistant' ? 'assistant-message' : 'user-message';
    div.textContent = text;
    messages.appendChild(div);
    messages.scrollTop = messages.scrollHeight;
  }

  // Respuestas locales del asistente
  function getAssistantResponse(userText) {
    const txt = userText.trim().toLowerCase();
    if (!txt) return '¿En qué puedo ayudarte?';
    if (txt.includes('medicamento') && txt.includes('agregar')) {
      return 'Para agregar un medicamento, busca el botón "Agregar medicamento" o similar en la pantalla principal y completa los datos requeridos.';
    }
    if ((txt.includes('marcar') || txt.includes('tomar')) && txt.includes('medicamento')) {
      return 'Para marcar un medicamento como tomado, haz clic en el botón "Marcar como tomado" junto al medicamento correspondiente.';
    }
    if (txt.includes('cerrar sesión') || txt.includes('logout')) {
      return 'Para cerrar sesión, busca la opción "Cerrar sesión" en el menú principal o en la parte superior de la pantalla.';
    }
    if (txt.includes('recordatorio')) {
      return 'Puedes gestionar tus recordatorios desde la sección de medicamentos. Cada medicamento puede tener recordatorios asociados.';
    }
    if (txt.includes('eliminar') && txt.includes('medicamento')) {
      return 'Para eliminar un medicamento, busca el botón de eliminar (generalmente un ícono de papelera) junto al medicamento.';
    }
    if (txt.includes('ayuda')) {
      return 'Soy tu asistente virtual. Puedo ayudarte con preguntas sobre cómo usar la aplicación. Pregúntame cómo agregar, marcar o eliminar medicamentos, o cómo cerrar sesión.';
    }
    return 'Lo siento, no tengo información sobre eso. ¿Puedes preguntar de otra manera o ser más específico?';
  }
